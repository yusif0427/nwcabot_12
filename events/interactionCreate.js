...
const moment = require("moment");
  ...
  if(interaction.isChatInputCommand()) {
    ...
    const cmd = client.commandMap ? client.commandMap.get(interaction.commandName.toLowerCase()) : null;
    if (!cmd) return;

    console.log("Komut kullandı: " + interaction.user.tag + " (" + interaction.user.id + ") (" + interaction.guild.name + ") ");

    try {
      await cmd.run(client, interaction, db);
    } catch (err) {
      console.error("Komut hatası:", err);
      if (!interaction.replied && !interaction.deferred) {
        await interaction.reply({ content: "Bu komutta bir hata oluştu.", ephemeral: true });
      }
    }
    return;
  }
